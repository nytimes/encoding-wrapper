...
type ErrNoPresetFound struct {
	PresetID string
}

func (e ErrNoPresetFound) Error() string {
	return fmt.Sprintf("no preset found with id: %s", e.PresetID)
}
...
type VideoTarget struct {
	...
	GopSize         int    `json:"exact_gop_frames,omitempty"`
	GopMode         bool   `json:"use_closed_gop,omitempty"`
